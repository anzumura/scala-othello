...
class GameState(val totalBlack: Int, val totalWhite: Int, val board: Board) {
  ...
  def gameOver: Boolean = totalBlack == 0 || totalWhite == 0 || (totalWhite + totalBlack) == 64
  ...
  def getPoints(color: Int): Int = {
    if (color == Board.Black) totalBlack - totalWhite else totalWhite - totalBlack
  }
...
object GameState {
  ...
  private val horizontal = for (i <- C.id to F.id by 2; j <- IndexedSeq(0, 7)) yield (i, j)
  private val vertical = for (i <- IndexedSeq(A.id, H.id); j <- 2 to 5) yield (i, j)
  private val center = for (i <- C.id to F.id by 2; j <- 2 to 5) yield (i, j)
  private val nextToHorizontal = for (i <- C.id to F.id by 2; j <- IndexedSeq(1, 6)) yield (i, j)
  private val nextToVertical = for (i <- IndexedSeq(B.id, G.id); j <- 2 to 5) yield (i, j)
  private val nextToCorner = IndexedSeq((A.id, 1), (A.id, 6), (B.id, 0), (B.id, 1),
    (B.id, 6), (B.id, 7), (G.id, 0), (G.id, 1), (G.id, 6), (G.id, 7), (H.id, 1), (H.id, 6))
  // create 'allMoves' with potentially best moves first and use this for generating validMoves
  // to increase pruning with alphaBeta. Tests at 6 ply showed full game time fall from 43 secs
  // to 13 for black and 56 to 14 for white - so 3 to 4 times lower than scanning from A1 to H8
  ...
  private def checkValid(board: Board, col: Int, row: Int): Boolean = {
    (if (col < C.id)
      ...
      checkHorizontal(board, col, row, 2) || checkHorizontal(board, col, row, -2)) ||
      (row match {
  ...
  private def checkHorizontal(board: Board, col: Int, row: Int, horizontal: Int) = {
  ...
  private def check(board: Board, col: Int, row: Int, horizontal: Int, vertical: Int): Boolean = {
    doCheck(board, col + horizontal, row + vertical, horizontal, vertical)
  }

  private def doCheck(board: Board, col: Int, row: Int, horizontal: Int, vertical: Int): Boolean = {
    // make sure the next in this direction is the other color and then start looking for my color
    board.getCell(col, row) == board.otherColor &&
      findSameColor(board, col + horizontal, row + vertical, horizontal, vertical)
  }

  @tailrec
  private def findSameColor(board: Board, col: Int, row: Int, horizontal: Int, vertical: Int): Boolean = {
