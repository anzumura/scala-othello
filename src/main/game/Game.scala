...
object Game {
  ...
  private val players = for (c <- PlayerColor.values.toList) yield createPlayer(c)
  ...
  private def makeMove(): Boolean = {
    ...
    // if there are no valid moves flip the color and see if the other player has any
    if (!state.hasMoves)
