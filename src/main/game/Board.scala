...
class Board {
  ...
  private def flipDirection(col: Int, row: Int, horizontal: Int, vertical: Int): Unit = {
    doFlipDirection(col + horizontal, row + vertical, horizontal, vertical)
  }

  private def doFlipDirection(col: Int, row: Int, horizontal: Int, vertical: Int): Unit = {
    if (getCell(col, row) != turn && flipFound(col + horizontal, row + vertical, horizontal, vertical))
      flipCell(col, row)
  }

  private def flipFound(col: Int, row: Int, horizontal: Int, vertical: Int): Boolean = {
    if (col < A.id || col > H.id || row < 0 || row > 7)
      return false
    val cell = getCell(col, row)
    // while the cells are the opposite color then recurse until my color is found
    if (cell == otherColor) {
...
object Board {
  // create a Board populated from an Array of Array[Int] (not all rows need to be specified)
